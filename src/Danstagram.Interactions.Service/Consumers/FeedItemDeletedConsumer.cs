...
namespace Danstagram.Interactions.Service.Consumers
{
  public class FeedItemDeletedConsumer : RepositoryConsumer<FeedItem>, IConsumer<FeedItemDeleted>
  {
    ...
    {
      FeedItemDeleted message = context.Message;
      ...
      System.Collections.Generic.IReadOnlyCollection<Like> likes = await likesRepository.GetAllAsync(like => like.FeedItemId == message.Id);
      System.Collections.Generic.IReadOnlyCollection<Comment> comments = await commentsRepository.GetAllAsync(comment => comment.FeedItemId == message.Id);
